...
with open('doubleintegrator.json', 'r') as json_file:
    keras_model_json = json_file.read()
keras_model = model_from_json(keras_model_json)
keras_model.load_weights("doubleintegrator.h5")
...
if u_limits:
    torch.save(full_step.state_dict(), 'doubleintegrator.pt')
else:
    torch.save(full_step.state_dict(), 'doubleintegrator_ulimits1.pt')
