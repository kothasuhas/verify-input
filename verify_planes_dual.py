...
for c in tqdm(cs):
    try:
        ...
        lambda1 = m.addMVar(shape=HIDDEN_DIM1, lb=-1e30, ub=1e30, name="lambda1")
        lambda2 = m.addMVar(shape=HIDDEN_DIM2, lb=-1e30, ub=1e30, name="lambda2")
        lambda3 = m.addMVar(shape=HIDDEN_DIM2, lb=-1e30, ub=1e30, name="lambda3")
        lambda4 = m.addMVar(shape=OUTPUT_DIM , lb=-1e30, ub=1e30, name="lambda4")

        obj_terms = []

        # p^Tx - q^T(ReLU(x)) pattern
        
        for i in range(HIDDEN_DIM1):
            lb = m.addVar(lb=-1e30, ub=1e30)
            obj_terms.append(lb)
            m.addConstr(lb <= 0)
            m.addConstr(lb <= (lambda0[i] * U - lambda1[i] * U))
            m.addConstr(lb <= lambda0[i] * L)

        for i in range(HIDDEN_DIM2):
            lb = m.addVar(lb=-1e30, ub=1e30)
            obj_terms.append(lb)
            m.addConstr(lb <= 0)
            m.addConstr(lb <= (lambda2[i] * U - lambda3[i] * U))
            m.addConstr(lb <= lambda2[i] * L)
        ...
        for i in range(INPUT_DIM):
            lb = m.addVar(lb=-1e30, ub=1e30)
            obj_terms.append(lb)
            m.addConstr(lb <= (c[i] - dotproduct(lambda0, weight0[:,i], HIDDEN_DIM1)) * -2)
            m.addConstr(lb <= (c[i] - dotproduct(lambda0, weight0[:,i], HIDDEN_DIM1)) * 2)
